...
센서 데이터는 시스템 오퍼레이션의 파라미터(`sideStatus(available)` 등)로 전달된다.

| 클래스 | 단일 책임 |
|--------|----------|
| User | 시스템을 시작시키는 주체 |
| ObstacleSensor | 장애물 감지 이벤트를 RVCController에 전달한다 |
...
| 클래스 | 단일 책임 |
|--------|----------|
| RVCController | 시스템 오퍼레이션(start, frontObstacleDetected, allSidesBlocked, sideStatus, dustDetected)을 수신하고 NavigationController·CleaningController에 위임한다 |
| NavigationController | DriveMotor를 제어한다. 방향 선택 로직(양쪽 여유 시 랜덤)을 포함한다 |
