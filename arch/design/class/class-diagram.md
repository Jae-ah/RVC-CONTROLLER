...
classDiagram
    ...
    class RVCController {
        ...
        +sideStatus(available: List~Direction~) void
        +allSidesBlocked() void
        +dustDetected() void
    }
    ...
    class NavigationController {
        ...
        +moveBackward() void
        +stop() void
        +turn(available: List~Direction~) void
    }
...
| ObstacleSensor | `frontObstacleDetected()`, `sideStatus(available)`, `allSidesBlocked()` 이벤트를 전달한다 |
...
| 클래스 | 책임 |
|--------|------|
| RVCController | 시스템 오퍼레이션을 수신하고 NavigationController·CleaningController에 위임한다 |
| NavigationController | `IDriveMotor`를 제어한다. `turn(available)`에서 `Direction` 목록을 받아 방향 선택 로직(양쪽 여유 시 랜덤)을 수행한다 |
...
| `RVCController.sideStatus(available: List<Direction>)` | SD-002, SD-003 |
| `RVCController.allSidesBlocked()` | SD-003 |
...
| `NavigationController.turn(available: List<Direction>)` | SD-002, SD-003 |
