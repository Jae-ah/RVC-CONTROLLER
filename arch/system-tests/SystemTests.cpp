...
// 시스템이 올바른 이벤트 시퀀스에서 기대 동작을 수행하는지 검증한다.
// ══════════════════════════════════════════════════════════════════════════
...
// ST-P05: 장애물 후 좌·우 모두 여유 — turn 1회, 유효 방향
static bool P05(std::string& failMsg) {
    RVCSimulator sim;
    sim.start();
    sim.frontObstacleDetected();
    sim.sideStatus({D::LEFT, D::RIGHT});

    SCHECK_EQ(sim.drive().turnCalls.size(), 1u, "turn 정확히 1회");
    bool valid = (sim.drive().turnCalls[0] == D::LEFT ||
                  sim.drive().turnCalls[0] == D::RIGHT);
    SCHECK(valid, "방향이 LEFT 또는 RIGHT이어야 함");
    return true;
}
...
static bool P06(std::string& failMsg) {
    RVCSimulator sim;
    sim.start();
    sim.resetRecording(); // start() 기록을 제거해 회피 시퀀스만 검사
    sim.allSidesBlocked();
    sim.sideStatus({D::RIGHT});
    ...
    SCHECK_EQ(sim.drive().stopCount,     1, "DriveMotor stop 1회");
    ...
    // 순서: moveForward 가 startCleaning(NORMAL) 보다 먼저 (SD-003 설계 노트)
...
// ST-P08: 삼면 장애물 — 빈 sideStatus 반복 후 방향 확보
static bool P08(std::string& failMsg) {
    RVCSimulator sim;
    sim.start();
    sim.allSidesBlocked();
    sim.sideStatus({});    // 여전히 막힘
    sim.sideStatus({});    // 여전히 막힘
    sim.sideStatus({D::RIGHT}); // 확보

    SCHECK_EQ(sim.drive().backwardCount, 1, "moveBackward 1회");
    SCHECK_EQ(sim.drive().turnCalls.size(), 1u, "turn 1회");
    SCHECK_EQ(sim.drive().stopCount, 1, "stop 1회");
    return true;
}
...
static bool P10(std::string& failMsg) {
    ...
    sim.start();
    sim.resetRecording(); // start()의 NORMAL 기록 제거
    sim.dustDetected();
...
// ST-N03: CLEANING에서 sideStatus({LEFT}) → turn 미호출 (장애물 이벤트 없이)
static bool N03(std::string& failMsg) {
    RVCSimulator sim;
    sim.start();
    sim.resetRecording();
    sim.sideStatus({D::LEFT}); // CLEANING 상태
    SCHECK_EQ(sim.drive().turnCalls.size(), 0u, "CLEANING에서 turn 미호출");
    return true;
}

// ST-N04: CLEANING에서 sideStatus({}) → 모터 추가 호출 없음
...
// ST-N05: REVERSING에서 sideStatus({}) → stop/turn/forward 미호출
static bool N05(std::string& failMsg) {
    RVCSimulator sim;
    sim.start();
    sim.allSidesBlocked();
    sim.resetRecording();
    sim.sideStatus({}); // 빈 리스트 → 전환 없음
    SCHECK_EQ(sim.drive().stopCount,        0, "stop 미호출");
...
// ST-N06: REVERSING에서 sideStatus({}) 3회 반복 → forward 총 0회 (후진만 유지)
...
// ST-N07: boostCleaning 연속 2회 → startCleaning(HIGH) 1회만
static bool N07(std::string& failMsg) {
    RVCSimulator sim;
    sim.start();
    sim.resetRecording();
    sim.dustDetected(); // HIGH + 타이머 시작
    sim.dustDetected(); // 타이머 리셋만, motor 재호출 없음
...
// ST-N08: dustDetected → DriveMotor에 어떤 호출도 없음 (주행 무중단)
...
// ST-N09: frontObstacleDetected → CleaningMotor.startCleaning 미호출 (정지만)
...
// ST-N10: allSidesBlocked → CleaningMotor.startCleaning 미호출
...
// ST-N11: sideStatus({LEFT, RIGHT}) → turn 정확히 1회 (2회 아님)
...
// ST-N15: start() 후 DriveMotor.stop 미호출 (아무 장애물 이벤트 없음)
...
// ST-N17: STOPPED 상태에서 allSidesBlocked → REVERSING 전환
//         stopCleaning 2회 (이미 정지된 상태에서 다시 정지), moveBackward 1회
static bool N17(std::string& failMsg) {
    RVCSimulator sim;
    sim.start();
    sim.frontObstacleDetected(); // STOPPED
    sim.allSidesBlocked();       // REVERSING (stopCleaning 두 번째 발생)
    SCHECK_EQ(sim.cleaning().stopCount, 2, "stopCleaning 2회");
    SCHECK_EQ(sim.drive().backwardCount, 1, "moveBackward 1회");
    return true;
}

// ST-N18: REVERSING 중 frontObstacleDetected → STOPPED 전환
//         이후 sideStatus({LEFT}) 가 정상적으로 회피 처리됨
static bool N18(std::string& failMsg) {
    RVCSimulator sim;
    sim.start();
    sim.allSidesBlocked();        // REVERSING
    sim.frontObstacleDetected();  // STOPPED (stopCleaning 한 번 더)
    sim.resetRecording();
    sim.sideStatus({D::LEFT});    // STOPPED → CLEANING
...
// ST-N19: 장애물 회피 두 번 연속 → turn 총 2회 (각 회피마다 1회)
...
// ST-N20: 고출력 중 장애물 감지 → DriveMotor stop 정확히 1회 (forward 중지)
//         CleaningMotor: HIGH 이후 stopCleaning, 그 다음 NORMAL (회피 후 재개)
static bool N20(std::string& failMsg) {
    RVCSimulator sim;
    sim.start();
    sim.resetRecording(); // start()의 NORMAL 기록 제거
    sim.dustDetected();           // HIGH
    sim.frontObstacleDetected();  // stopCleaning, DriveMotor stop
    sim.sideStatus({D::RIGHT});   // 회피 → NORMAL + forward
...
int main() {
    ...
    runner.add("ST-P05", POS, "장애물 회피 좌·우 모두 여유 — turn 1회, 유효 방향", P05);
    runner.add("ST-P06", POS, "삼면 장애물 후 우측 전진 — 순서 포함",             P06);
    runner.add("ST-P07", POS, "삼면 장애물 후 좌측 전진 — 순서 포함",             P07);
    runner.add("ST-P08", POS, "삼면 장애물 — 빈 sideStatus 반복 후 방향 확보",    P08);
    ...
    runner.add("ST-N02", NEG, "IDLE에서 sideStatus({}) — 모든 모터 미호출",          N02);
    runner.add("ST-N03", NEG, "CLEANING에서 sideStatus({LEFT}) — turn 미호출",       N03);
    runner.add("ST-N04", NEG, "CLEANING에서 sideStatus({}) — 모터 추가 호출 없음",   N04);
    runner.add("ST-N05", NEG, "REVERSING에서 sideStatus({}) — stop/turn/forward 없음", N05);
    runner.add("ST-N06", NEG, "REVERSING에서 sideStatus({}) 3회 — forward 0회",      N06);
    ...
    runner.add("ST-N10", NEG, "allSidesBlocked — startCleaning 미호출",               N10);
    runner.add("ST-N11", NEG, "sideStatus({L,R}) — turn 정확히 1회",                 N11);
    ...
    runner.add("ST-N17", NEG, "STOPPED → allSidesBlocked — stopCleaning 2회",        N17);
