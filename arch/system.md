...
| SR-02 | 청소하면서 직진한다 |
| SR-03 | 센서가 장애물을 감지하면 청소를 멈추고, 좌/우로 방향을 틀어 청소하며 전진한다 |
...
2. **장애물 회피 (측면 여유 있음)**: 전방 장애물 감지 → 청소 정지 → 좌 또는 우로 회전 → 청소 재개 및 전진.
3. **장애물 회피 (삼면 막힘)**: 전/좌/우 장애물 모두 감지 → 후진(이때 좌 또는 우에 막히지 않는 방향이 나올 때까지 후진한다) → 좌 또는 우로 회전 → 전진.
...
| 단계 | 산출물 |
|------|--------|
| 요구사항 분석 | arch/system.md, arch/requirements/fi-nfi.md |
