...
| 확장 포인트 | 조건 | 확장 UC |
|------------|------|---------|
| EP-1 | frontObstacleDetected AND (좌 OR 우 여유 있음) | UC-002 |
| EP-2 | allSidesBlocked | UC-003 |
...
flowchart TD
    ...
    B --> C{이벤트 감지?}
    C -- frontObstacleDetected AND 측면 여유 --> EP1(["EP-1: UC-002 확장"])
    C -- allSidesBlocked --> EP2(["EP-2: UC-003 확장"])
